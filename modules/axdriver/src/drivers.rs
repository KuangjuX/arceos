...
use crate::ixgbe::pci_probe_ixgbe;
...
cfg_if::cfg_if! {
    if #[cfg(net_dev = "ixgbe")] {
        ...
        const QN: u16 = 1;
        const QS: usize = 1024;
        register_net_driver!(IxgbeDriver, driver_net::ixgbe::IxgbeNic<IxgbeHalImpl, {QS}, {QN}>);
        impl DriverProbe for IxgbeDriver {
            fn probe_pci(
                    ...
                    if let Some(ixgbe_nic) = pci_probe_ixgbe(root, bdf, dev_info) {
                        info!("register a new ixgbe device.");
